...
class TestJinaRerankerInit:
    ...
    def test_stores_api_key(self) -> None:
        r = JinaReranker(api_key="secret-key")
        assert r._api_key == "secret-key"
...
class TestJinaRerankerRerank:
    async def test_successful_rerank(self) -> None:
        ...
        assert call_kwargs.kwargs["json"]["top_n"] == 2
        assert (
            call_kwargs.kwargs["json"]["model"] == "jina-reranker-v2-base-multilingual"
        )
...
class TestSearchWithReranker:
    async def test_reranker_called_and_results_reordered(self) -> None:
        ...
        # FakeReranker reverses order: "third" (idx 2) gets 0.9, "second" (idx 1) gets 0.8, "first" (idx 0) gets 0.7
        # After recency/access boost the order should still reflect reranker output
    ...
    async def test_reranker_not_called_when_none(self) -> None:
        ...
        # Results should preserve RRF order (after recency/access boost)
    ...
    async def test_reranker_scores_feed_into_recency_boost(self) -> None:
        ...
        # The final score should be based on 0.9, not the original 0.3
        assert len(results) == 1
        assert results[0].score > 0.85  # 0.9 after boosts stays high
