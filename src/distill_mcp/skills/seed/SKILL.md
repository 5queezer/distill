---
name: seed
description: Populate the distill knowledge base from a repo's git history with proper timestamps, metadata validation, and spot-checks. Improved version of seed-from-git that addresses common seeding gaps.
---
...
Report the repo name and commit count to the user. Ask if they want to seed all commits or a date range.
...
- **occurred_at**: The commit's author date (ISO 8601 from `%aI`). NEVER omit this. NEVER use current time.

When a later commit revises an earlier decision, reference the evolution:
> "Initially chose SQLite for storage (2024-01). Switched to PostgreSQL + pgvector (2024-03) to enable stateless deployment on GKE."
...
Check that:
- [ ] `occurred_at` is the commit date, not today's date
...
1. **Using current time instead of commit date** — this is the #1 seeding bug. Always use `%aI` from git log.
2. **Storing implementation details** — "Added postgres_store.py with asyncpg pool" is useless. "Chose PostgreSQL over SQLite for stateless pod deployment" is valuable.
3. **Missing the WHY** — if the commit message doesn't explain why, check the PR body or surrounding commits for context.
4. **Not connecting related commits** — a decision and its later reversal should reference each other.
5. **Over-seeding noise** — skip formatting, linting, and trivial fixes. Quality over quantity.
